...
graph_data = health_data %>%
  filter(`연령대코드(5세단위)` >= 3 & `연령대코드(5세단위)` <= 6) %>%
           group_by(성별) %>%
           summarise(MEAN_ATL = mean(`혈청지피티(ALT)`, na.rm = TRUE)) %>%
           mutate(Gender = ifelse(성별 == 1,"Male","Female"))
  
# View(graph_data)
p = ggplot(data = graph_data, aes(x = 성별,y = MEAN_ATL,fill = Gender))+
  geom_col()+
  scale_fill_manual(values = c("Male" = "blue", "Female" = "red"))+ # 색상 수동 지정
  labs(
    title = "성별 평균 혈청지피티",
    x = "성별",
    y = "평균 ATL"
