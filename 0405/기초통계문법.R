...
# install.packages('DescTools') # 최빈값 구해주는 기능, ctrl + enter <- 한줄만 실행, 주석처리
library(DescTools) # 경고메시지 무시

# 최빈값
# 최빈값은 직원들 급여가 어디에 분포가 되어있는지 알고싶을 때
value = Mode(data)
cat('최빈값 : ', value, '\n')
...
# 절대값을 사용하면 되지않나?
# 절대값은 미분이 불가능. 제곱된 미분과 같은 수학적 조작이 가능해
# 통계 이론 전개에 유리합니다.
variance = var(data)
cat('분산 : ', variance, '\n')
...
# 임시 데이터 만들기
# rnorm : 임시 데이터 만들기
people = rnorm(1000, mean = 170, sd = 10) # 1000명 평균키 150, 표준오차 10
# print(people) <- 데이터 많아서 주석처리
