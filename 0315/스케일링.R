# *****데이터스케일링
# `데이터분석` 및 `머신러닝`에서 중요한 전처리 과정
# 변수의 크기를 조정하여 성능을 향상시키거나 결과를
# 해석하기 쉽게 만든다.

# Min-Max 정규화
# 데이터 스케일링 방법 중 하나(대표적인)
# Min-Max는 데이터를 0에서 1사이로 변환하는
# 데이터 전처리 기법

# 예제 데이터 프레임
data = data.frame(
  hight = c(150, 160, 170, 180, 190),
  weight = c(50, 60, 70, 80, 90)
)
# View(data)
# 키와 몸무계는 단위와 범위가 다르기 때문에
...
heigh_min = min(data$hight) # 키 최솟값
heigh_max = max(data$hight) # 키 최댓값
# 키 스케일링
data$scaled_height = (data$hight -heigh_min) /
  (heigh_max - heigh_min)

weight_min = min(data$weight) # 몸무게 최솟값
weight_max = max(data$weight) # 몸무게 최댓값

# 몸무게 스케일링
data$scaled_weight = (data$weight - weight_min) /
  (weight_max - weight_min)
# View(data)  <- data$scaled_weight 이것이 아니라 data 이것임 $ 때문

# emp 데이터에서 급여(SAL) 열에 대해 Min-max
# 정규화를 수행해주세요.(SAL_MinMax 컬럼 추가)
