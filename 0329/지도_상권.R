...
# 왜? 병합(join)하기 위해서
# 지도데이터(shp)와 상군데이터(csv) 병합하기

# seoul_map에 있는 SIG_SD와
...
# 그룹핑 해서 전체 `페업 영업 개월 평균`의 평균 구하기
...
# quartiles : 사분위수(데이터를 4등분 하는 기준 값)
# 25%, 50%, 75%
quartiles = quantile(merged_data$영업평균)
print(quartiles) # 사분위수 결과 확인

merged_data = merged_data %>%
  filter(영업평균 >= quartiles["75%"])
...
p = ggplot(data = merged_data) +
  scale_fill_gradient(low = "#ececec",
                      ...
                      name = "영업평균") + # 지도 색깔 설정
  geom_sf_interactive(
    aes(fill = 영업평균, # 영업 평균 데이터를 지도에 채우기
        tooltip = 자치구_코드_명,
        data_id = SIG_CD)
  ) + # 마우스 호버 이벤트
  theme_minimal() +
  labs(title = "서울시 페업 평균 개월",
       x = "경도",
       y = "위도")
girafe_plot = girafe(ggobj = p) # 인터렉티브 지도 생성
